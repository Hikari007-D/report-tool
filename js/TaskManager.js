...
class TaskManager {
  ...
  /**
   * Insert a task at specific index (for undo)
  ...
  /**
   * Move task from one index to another (for drag & drop)
   * @param {number} from - Source index
   * @param {number} to - Destination index
   */
  moveTaskTo(from, to) {
    if (from < 0 || from >= this.tasks.length) return;
    if (to < 0 || to > this.tasks.length) return; // 'to' can be tasks.length for appending
    const [task] = this.tasks.splice(from, 1);
    this.tasks.splice(to, 0, task);
    this.notifyListeners();
  }
  ...
  isEmpty() {
    return (
      this.tasks.length === 0 || this.tasks.every((task) => !task.detail.trim())
    );
  }
